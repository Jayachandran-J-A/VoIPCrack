...
def extract_voip_audio(pcap_file, output_file):
    """
    Extract and decode VoIP audio from G.729 RTP packets.
    """
    ...
    for stream_key, packets in rtp_streams.items():
        try:
            ...
            for packet in sorted_packets:
                g729_data += packet['payload']
            
            process = subprocess.run([
                "ffmpeg", "-y", "-f", "g729", "-i", "-", output_file
            ], input=g729_data, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
